function savePassword() {
    ...
    fetch(`/api/passwords/save?username=${encodeURIComponent(username)}&service=${encodeURIComponent(service)}&password=${encodeURIComponent(password)}`, {
        method: 'POST'
    })
    .then(res => res.text())
    .then(msg => {
...
function getPassword() {
    ...
    fetch(`/api/passwords/get?username=${encodeURIComponent(username)}&service=${encodeURIComponent(service)}`)
        .then(res => res.text())
        .then(pwd => {
